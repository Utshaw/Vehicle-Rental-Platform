...
class DAO
{
    ...
    public function getRecommendationMatrix($customer)
    {
        ...
        global $pdo;
        $sql = "SELECT * FROM VEHICLE AS V JOIN VEHICLE_ORDER AS VOR JOIN VEHICLE_COMPANY AS VC JOIN VEHICLE_MAKE AS VMK JOIN VEHICLE_MODEL AS VM ON V.COMPANY_ID=VC.COMPANY_ID AND V.MAKE_ID = VMK.MAKE_ID  AND V.VEHICLE_ID = VOR.VEHICLE_ID AND   V.MODEL_ID=VM.MODEL_ID  WHERE CUSTOMER_ID = :customer_id ORDER BY VOR.ORDER_ID DESC LIMIT 4 ";
